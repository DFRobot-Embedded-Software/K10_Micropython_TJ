...
inline void gpio_intr_handler_register(void (*fn)(void*), void *arg){}
...
// ESP-IDF API compatibility fixes for v5.x

// Fix for deprecated get_sp - always define it
...
#ifdef ESP_PLATFORM
    // Fix for deprecated ets_write_char_uart
    #ifndef ets_write_char_uart
        #define ets_write_char_uart(c) putchar(c)
    #endif

    // Fix for deprecated ets_printf
    #ifndef ets_printf
        #define ets_printf printf
    #endif

    // Fix for deprecated gpio_intr_handler_fn_t
    #ifndef gpio_intr_handler_fn_t
        typedef void (*gpio_intr_handler_fn_t)(void* arg);
    #endif
#endif
...
static inline void cb_isr(mp_obj_t cb, mp_obj_t arg)
{
    if (cb != NULL && cb != mp_const_none) {
        ...
        mp_stack_set_limit(CONFIG_NIMBLE_TASK_STACK_SIZE - 1024); // tune based on ISR thread stack size
